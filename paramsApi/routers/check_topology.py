...
from ..services import check_topology_srv
import pytz
import logging
...
def get_boards(req: schemas.GetBoardRequest):
    ...
    total = check_topology_srv.get_total(req.filter)
    return schemas.GetCheckTopologyResponse(
            data=schemas.CheckTopologyItem(
                myArray=check_topology_srv.get_by_page(req.page_index_begin, req.page_size, req.filter),
...
def export_data(req: schemas.ExportRequest):
    return schemas.FilterResponse(
        data=check_topology_srv.get_all(req.filter)
    )
