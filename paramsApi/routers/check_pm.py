...
from .. import schemas
from ..services import check_pm_srv
import logging
...
def get_boards(req: schemas.GetBoardRequest):
    ...
    total = check_pm_srv.get_total(req.filter)
    return schemas.GetCheckTopologyResponse(
            data=schemas.CheckTopologyItem(
                myArray=check_pm_srv.get_by_page(req.page_index_begin, req.page_size, req.filter),
...
def export_data(req: schemas.ExportRequest):
    return schemas.FilterResponse(
        data=check_pm_srv.get_all(req.filter)
    )
