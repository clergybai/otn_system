...
export default {
  ...
  methods: {
    ...
    init() {
      ...
      getCheckOpticalNe(data0).then((res) => {
        if (res.code == 200) {
    ...
    changePage() {
      ...
      getCheckOpticalNe(obj).then((res) => {
        if (res.code == 200) {
          this.refreshTable(res.data);
        }
      });
    ...
    exportor() {
      ...
      new Promise((resolve, reject) => {
        exportCheckOpticalNeData(data0).then((res) => {
          if (res.code == 200) {
            const data = res.data;
            for (const item of data) {
              ...
              list.city = item.city;
              list.ne_name = item.ne_name;
              list.sub_name = item.sub_name;
              list.manufactor = item.manufactor;
              list.ne_type = item.ne_type;
              list.ne_model = item.ne_model;
            ...
            CsvExportor.downloadCsv(
              tableData,
              { header },
              "checkOpticalNe" + _this.dateFormat() + ".csv"
            );
            ElMessage({
              type: "success",
              message: "导出成功",
