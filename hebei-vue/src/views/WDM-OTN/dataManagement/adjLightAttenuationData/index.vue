    ...
    <div class="operatingHead">
      <div class="operatingTool">
        <el-form :inline="true" class="demo-form-inline">
          <el-form-item label="传输系统" :style="{ marginRight: '15px' }">
            <el-input
                  label="传输系统"
                  placeholder="传输系统"
          ...
          <el-form-item label="子网元名称" :style="{ marginRight: '15px' }">
            <el-input
                  label="子网元名称"
                  placeholder="子网元名称"
                  v-model="qry_sub_name"
                  :style="{ width: '120px' }"
                />
          </el-form-item>
          <el-form-item label="光放子架" :style="{ marginRight: '15px' }">
            <el-input
                  label="光放子架"
                  placeholder="光放子架"
                  v-model="qry_shelf_id"
                  :style="{ width: '120px' }"
                />
          </el-form-item>
          <el-form-item label="光放槽位" :style="{ marginRight: '15px' }">
            <el-input
                  label="光放槽位"
                  placeholder="光放槽位"
                  v-model="qry_slot_id"
          ...
          <el-form-item label="数据错误类型" :style="{ marginRight: '15px' }">
            <el-input
                  label="数据错误类型"
                  placeholder="数据错误类型"
          ...
          <el-form-item :style="{ marginRight: '15px' }">
...
export default {
  ...
  methods: {
    ...
    generate_filter() {
      ...
      if (this.qry_sub_name.trim() !== ""){
        filter.push({
          name: "sub_name",
          values: this.qry_sub_name
        });
      }
      if (this.qry_shelf_id.trim() !== ""){
        filter.push({
          name: "shelf_id",
          values: this.qry_shelf_id
        });
      }
      if (this.qry_slot_id.trim() !== ""){
        filter.push({
          name: "slot_id",
          values: this.qry_slot_id
        });
      }
    ...
    exportor() {
      ...
      new Promise((resolve, reject) => {
        exportCheckVoaData(data0).then((res) => {
          if (res.code == 200) {
            const data = res.data;
            for (const item of data) {
              let list = {};
              list.sys_name = item.sys_name;
              list.sub_name = item.sub_name;
              list.shelf_id = item.shelf_id;
              list.slot_id = item.slot_id;
              list.va4_shelf_id = item.va4_shelf_id;
              list.va4_slot_id = item.va4_slot_id;
              list.va4_port_id = item.va4_port_id;
