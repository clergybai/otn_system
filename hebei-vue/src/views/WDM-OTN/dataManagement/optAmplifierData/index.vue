    ...
    <div class="operatingHead">
      <div class="operatingTool">
        <el-form :inline="true" class="demo-form-inline">
          ...
          <el-form-item label="子架" :style="{ marginRight: '15px' }">
            <el-input
                  label="子架"
                  placeholder="子架"
          ...
          <el-form-item label="槽位" :style="{ marginRight: '15px' }">
            <el-input
                  label="槽位"
                  placeholder="槽位"
                  v-model="qry_slot_id"
...
export default {
  ...
  methods: {
    ...
    init() {
      ...
      getCheckOaBoardType(data0).then((res) => {
        if (res.code == 200) {
    ...
    changePage() {
      ...
      getCheckOaBoardType(obj).then((res) => {
        if (res.code == 200) {
          this.refreshTable(res.data);
        }
      });
    ...
    exportor() {
      ...
      new Promise((resolve, reject) => {
        exportCheckOaBoardTypeData(data0).then((res) => {
          if (res.code == 200) {
            const data = res.data;
            for (const item of data) {
              ...
              list.board_model = item.board_model;
            ...
            CsvExportor.downloadCsv(
              tableData,
              { header },
              "checkOaBoardType" + _this.dateFormat() + ".csv"
            );
            ElMessage({
              type: "success",
              message: "导出成功",
